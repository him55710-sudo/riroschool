...
function cleanSnippet(text, max = 600) {
    if (!text)
        return "";
    return text
...
function topicKeywords(topic, lang) {
    const raw = topic
        .toLowerCase()
        .split(/[^\w가-힣]+/g)
...
function scoreSentence(sentence, keywords) {
    ...
    if (/광고|sponsored|click|buy now/i.test(sentence))
        score -= 5;
    return score;
}
function buildSourceDigests(sources, job, lang) {
...
function pickEvidenceSentence(digests, cursor, offset = 0) {
    if (digests.length === 0) {
...
function fallbackSectionBlueprints(lang) {
    if (lang === "ko") {
        return [
...
function buildOverviewSection(job, digests, lang) {
    if (lang === "ko") {
...
function buildSourceMatrix(digests, lang) {
    if (lang === "ko") {
        let table = "## 출처 매트릭스\n";
...
function buildMermaidBlock(lang) {
    if (lang === "ko") {
        return [
            "## 실행 아키텍처",
            "```mermaid",
...
function buildSectionBody(job, digests, lang, sectionTitle, objective, sectionIndex, paragraphsPerSection) {
...
function buildRoadmapBlock(digests, lang) {
...
function buildAdvancedFallbackMarkdown(job, sources) {
...
function buildDeepDiveBlock(job, digests, lang, round) {
...
function isWeakDraft(markdown, job) {
    const target = TIER_TARGETS[job.tier] || TIER_TARGETS.FREE;
    const lang = normalizeLanguage(job.language);
...
function shouldBlendWithLocal(markdown, job) {
    const target = TIER_TARGETS[job.tier] || TIER_TARGETS.FREE;
    const lang = normalizeLanguage(job.language);
    const sections = countH2Sections(markdown);
...
function buildOllamaPrompt(job, lang, digests) {
    const targets = TIER_TARGETS[job.tier] || TIER_TARGETS.FREE;
...
async function generateWithGemini(prompt, geminiApiKey) {
    ...
    for (const modelName of models) {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            try {
                ...
                console.warn(`[WRITE] Gemini request failed for model '${modelName}' (attempt ${attempt + 1}/2): ${getErrorMessage(error).slice(0, 260)}`);
                break;
            }
        }
    }
...
function isOllamaExplicitlyEnabled() {
    if (process.env.OLLAMA_DISABLED === "1")
        return false;
    if ((process.env.OLLAMA_ENABLED || "0").trim() !== "1")
        return false;
...
function getOllamaNumPredict(job) {
    const override = Number(process.env.OLLAMA_NUM_PREDICT || "");
    if (Number.isFinite(override) && override > 0) {
        return Math.floor(override);
    }
    if (job.tier === "PREMIUM_PACK")
        return 6200;
    if (job.tier === "PRO_PACK")
        return 4600;
    return 3200;
}
async function generateWithOllama(job, lang, digests) {
...
async function generateDraft(job) {
    ...
    if (!adminConfig) {
        adminConfig = await shared_1.prisma.adminConfig.create({
    ...
    if (!markdown && ollamaEnabled) {
        ...
        console.warn(`[WRITE] Trying Ollama models: ${candidateModels.join(", ") || "(none)"} (num_predict=${getOllamaNumPredict(job)}).`);
    ...
    console.log(`[WRITE] Mode=${generationMode}, chars=${markdown.length}, h2=${countH2Sections(markdown)}, koreanRatio=${koreanRatio(markdown).toFixed(2)}`);
